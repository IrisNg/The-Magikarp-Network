<template>
  <div class="scheduler">
    ...
    >
      <slide v-for="(monthId,index) in months" :key="monthId">
        <calendar :month-id="monthId" :page-index="index" />
      </slide>
    </carousel>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    for (
      let i = currentMonthIndexInYear - range;
      i <= currentMonthIndexInYear + range;
      i++
    ) {
      let monthId = dayjs()
        .date(1)
  ...
  methods: {
    ...
    onPageChange(currentPage) {
      ...
      this.updateArrowLabels(currentPage);

      EventBus.$emit("scheduler-page-change", currentPage);
    },

    updateArrowLabels(currentPage) {
